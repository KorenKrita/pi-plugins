...
export function renderFixedEditorCluster(input: FixedEditorClusterInput): FixedEditorClusterRender {
  ...
  const footerSource = normalizeLines(input.footerLines, width);
  ...
  const footer = takeTail(footerSource, Math.max(0, maxRows - 1));
  const editorLines = capEditorLines(editorSource, maxRows - footer.length);
  let remaining = maxRows - editorLines.length - footer.length;
