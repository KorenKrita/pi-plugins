...
export class TerminalSplitCompositor {
  ...
  private readonly onCopySelection: ((text: string) => void | Promise<void>) | null;
  private readonly onCopyError: ((error: Error) => void) | null;
  private extendedKeyboardMode: ExtendedKeyboardMode | null = null;
  private readonly rowsLookupDescriptor: PropertyDescriptor | undefined;
  private readonly originalOwnRowsDescriptor: PropertyDescriptor | undefined;
  private readonly originalTerminalWrite: (data: string) => void;
  private readonly originalDoRender: (() => void) | null;
  private readonly originalRender: ((width: number) => string[]) | null;
  private readonly originalCompositeLineAt: CompositeLineAt | null;
  private rowsGetter: (() => number) | null = null;
  private terminalWritePatch: ((data: string) => void) | null = null;
  private doRenderPatch: (() => void) | null = null;
  private renderPatch: ((width: number) => string[]) | null = null;
  private compositeLineAtPatch: CompositeLineAt | null = null;
  ...
  private readonly agentResponseJump = new AgentResponseJumpState();
  private userViewportLocked = false;
  private viewportAnchor: ViewportAnchor | null = null;
  ...
  constructor(options: TerminalSplitCompositorOptions) {
    ...
    this.onCopyError = options.onCopyError ?? null;
    this.rowsLookupDescriptor = descriptorForRows(options.terminal);
    this.originalOwnRowsDescriptor = Object.getOwnPropertyDescriptor(options.terminal, "rows");
    this.originalTerminalWrite = options.terminal.write;
    this.originalDoRender = typeof options.tui.doRender === "function" ? options.tui.doRender : null;
    this.originalRender = typeof options.tui.render === "function" ? options.tui.render : null;
    this.originalCompositeLineAt = typeof options.tui.compositeLineAt === "function"
      ? options.tui.compositeLineAt
      : null;
  }
  ...
  install(): void {
    ...
    if (typeof this.terminal.write !== "function") {
      throw new Error("[fixed-input] Fixed editor compositor expected terminal.write(data) to exist");
    }
    ...
    if (this.originalRender) {
      this.renderPatch = (width: number) => this.renderScrollableRoot(width);
      this.tui.render = this.renderPatch;
    }
    ...
    this.terminalWritePatch = (data: string) => this.write(data);
    this.terminal.write = this.terminalWritePatch;
    if (this.originalDoRender) {
      ...
      this.tui.doRender = this.doRenderPatch;
    }
    if (this.originalCompositeLineAt) {
      this.compositeLineAtPatch = (
        baseLine: string,
        overlayLine: string,
        startCol: number,
        overlayWidth: number,
        totalWidth: number,
      ) => this.originalCompositeLineAt?.call(
        this.tui,
      ...
      this.tui.compositeLineAt = this.compositeLineAtPatch;
    }
    this.installed = true;
  }
  ...
  hideRenderable(target: PatchedRenderable): void {
    ...
    const hiddenRender = (_width: number) => [];
  ...
  beginAgentResponse(): void {
    this.agentResponseJump.beginTurn();
  }

  jumpToAgentResponseStartIfLong(): boolean {
    const shouldJump = this.agentResponseJump.completeTurn();
    if (!shouldJump || this.disposed || this.hasVisibleOverlay()) return false;

    const width = Math.max(1, this.terminal.columns || 80);
    this.refreshRootWindow(width);
    const range = findFinalAssistantTextRange(this.tui, width);
    if (!range) return false;

    const nextOffset = autoJumpOffsetForRange(
      range.startLine,
      range.endLine,
    ...
    if (nextOffset === null || nextOffset === this.scrollOffset) return false;

    this.clearSelection();
    this.lastLeftPress = null;
    this.scrollOffset = nextOffset;
    this.userViewportLocked = false;
  ...
  dispose(options: DisposeOptions = {}): void {
    ...
    if (this.terminalWritePatch && this.terminal.write === this.terminalWritePatch) {
      this.terminal.write = this.originalTerminalWrite;
    }
    if (this.doRenderPatch && this.tui.doRender === this.doRenderPatch && this.originalDoRender) {
      this.tui.doRender = this.originalDoRender;
    }
    if (this.renderPatch && this.tui.render === this.renderPatch && this.originalRender) {
    ...
    const currentRowsDescriptor = Object.getOwnPropertyDescriptor(this.terminal, "rows");
    if (this.rowsGetter && currentRowsDescriptor?.get === this.rowsGetter) {
    ...
    this.rowsGetter = null;
    this.terminalWritePatch = null;
    this.doRenderPatch = null;
    this.renderPatch = null;
    this.compositeLineAtPatch = null;

    this.restoreTerminalState(options);
  }

  private getRawRows(): number {
    return Math.max(1, readRows(this.terminal, this.rowsLookupDescriptor));
  }
  ...
  private renderScrollableRoot(width: number): string[] {
    if (!this.originalRender || this.disposed || this.renderingScrollableRoot) {
      return this.originalRender?.call(this.tui, width) ?? [];
    }

    if (this.hasVisibleOverlay()) {
      return this.originalRender.call(this.tui, width).map((line) => sanitizeOverlayBaseLine(line, Math.max(1, width)));
    }
  ...
  private refreshRootWindow(width: number): number {
    ...
    const lines = this.originalRender.call(this.tui, renderWidth);
    this.rootLines = lines;
    if (this.userViewportLocked && this.viewportAnchor && this.lastRootLineCount > 0) {
      const resolved = resolveViewportAnchor(this.viewportAnchor, lines, scrollableRows);
      this.scrollOffset = scrollOffsetForViewportStart(
        resolved.startLine,
        lines.length,
        scrollableRows,
      );
      if (resolved.matched) {
        this.viewportAnchor = captureViewportAnchor(lines, resolved.startLine, scrollableRows);
      }
    } else if (this.scrollOffset > 0 && this.lastRootLineCount > 0 && lines.length > this.lastRootLineCount) {
  ...
  private handleInput(data: string): { consume?: boolean; data?: string } | undefined {
    ...
    if (mousePackets) {
      ...
      for (const packet of mousePackets) {
        handled = this.handleMousePacket(packet) || handled;
      }
    ...
    this.agentResponseJump.markUserScrollIntent();
  ...
  private scrollSelectionAtViewportEdge(packet: SgrMousePacket): boolean {
    ...
    this.preserveSelectionFocusOnRelease = true;
    this.scrollOffset = nextOffset;
    this.userViewportLocked = nextOffset > 0;
    this.pendingImageCleanup = true;
    const start = this.updateVisibleRootWindow();
    this.viewportAnchor = this.userViewportLocked
      ? captureViewportAnchor(this.rootLines, start, this.visibleScrollableRows)
      : null;
  ...
  private scrollBy(delta: number): void {
    ...
    this.repaintScrollableViewport(width);
    this.viewportAnchor = this.userViewportLocked
      ? captureViewportAnchor(this.rootLines, this.visibleRootStart, this.visibleScrollableRows)
      : null;
    this.requestRender();
  }

  private copySelection(text: string): void {
    if (!this.onCopySelection) return;
    try {
      void Promise.resolve(this.onCopySelection(text)).catch((error) => {
  ...
  private writeOriginal(data: string): void {
    this.originalTerminalWrite.call(this.terminal, data);
  }
  ...
  private restoreTerminalState(options: DisposeOptions = {}): void {
    ...
    const restoreMainScreenMode = !options.resetExtendedKeyboardModes
      && activeMode !== null
      && (this.extendedKeyboardMode === null || this.extendedKeyboardMode === "modifyOtherKeys");
  ...
  private hasVisibleOverlay(): boolean {
    ...
    try {
      const getTopmostVisibleOverlay = Reflect.get(this.tui, "getTopmostVisibleOverlay");
      if (typeof getTopmostVisibleOverlay === "function") {
        return Boolean(getTopmostVisibleOverlay.call(this.tui));
      }
      ...
      const width = Math.max(1, this.terminal.columns || 80);
      const height = this.getRawRows();
      return overlayStack.some((entry) => {
        if (!entry || entry.hidden === true || entry.options?.nonCapturing === true) return false;
        const visible = entry.options?.visible;
        return typeof visible !== "function" || visible(width, height);
      });
    } finally {
