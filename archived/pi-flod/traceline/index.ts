...
import { rawIndexAtVisibleIndex, rawIndexBeforeVisibleIndex, stripAnsi } from "./ansi.ts";
import { positiveNumberValue, isJsonObject } from "./boundary.ts";
...
import { configPaths, readJsonConfig } from "./config.ts";
import { compactCount } from "./fmt.ts";
import {
  ELLIPSIS,
  GLYPH,
  SEP,
...
function toolPrefix(tone: Tone, _comp?: ToolRowDataLike): string {
  const bullet = ink(currentTheme(), tone, TOOL_BULLET);
  return `${TOOL_INDENT}${dim(TOOL_RAIL)} ${bullet}${TOOL_AFTER_BULLET}`;
}
...
// Shared path and truncation helpers from style.ts follow traceline's family rules
// (design language §5).
...
// Shared ANSI index helpers live in ansi.ts.
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    captureTui(ctx.ui, "__pi_traceline_capture", (tui) => {
      ...
      if (t.__tracelineRRWrapVersion !== TRACELINE_PATCH_VERSION) {
        // Pi 0.84 passes a stable renderer Proxy to widget factories. Capturing
        // t.requestRender directly would capture its dynamic forwarding closure and recurse
        // after this wrapper replaces the renderer method. Prefer Turn Fold's object bridge
        // when present; otherwise invoke the concrete renderer prototype implementation.
        const turnFoldBridge = (t as TracelineTuiLike & {
          __piFlodRenderBridge?: { requestRender?: (force?: boolean) => unknown };
        }).__piFlodRenderBridge;
        const rendererRequestRender = Reflect.get(Object.getPrototypeOf(t), "requestRender");
        const orig = turnFoldBridge?.requestRender ?? (
          typeof rendererRequestRender === "function"
            ? (force?: boolean) => Reflect.apply(rendererRequestRender, t, [force])
            : undefined
        );
    ...
    g.__tracelineInputUnsubscribe = ctx.ui.onTerminalInput((data) =>
      handleThinkingToggleTerminalInput(data, ctx.ui),
    );
  });
